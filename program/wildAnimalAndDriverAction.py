...
def wild_animal_report():
    ...
    if (prob_x_nca_prob_nca > prob_x_ca_prob_ca):
        print(" RESULT: Given the crash scenario X = [Weather Condition: Clear, Road Condition: Dry, Wild Animal: True] we predict there will be No Contributing Action from the driver.")
    else:
        print(" RESULT: Given the crash scenario X = [Weather Condition: Clear, Road Condition: Dry, Wild Animal: True] we predict there will be a Contributing Action from the driver.")
