...
def construction_zone_report():
    # Read in crash data
    crash_data = pd.read_csv("../assets/all_crash_data.csv")
    crash_data_frame = pd.DataFrame(crash_data)
    
    print("\n ----------------------------------------------------------------------------------")
    print(" >>> Classifier Driver Action in Construction Zone Crash Under Ideal Conditions <<< \n")

    # count num crashes and set up data frame
    num_crashes = crash_data["CUID"].count()

    # count num crashes involving a wild animal
    num_construction_crashes = crash_data["Construction Zone"].sum()
    prob_construction_crashes = (num_construction_crashes / num_crashes)

    print(" --- Some General Numbers --- \n")

    print(f" Number of crashes Analyzed: {num_crashes}")
    print(f" Number of crashes in a Construction Zone: {num_construction_crashes}")
    print(f" Probability of a crash in a Construction Zone: {prob_construction_crashes}\n")
    ...
    Construction Zone Crash Under Ideal Conditions:
    X = [Weather Condition: Clear, Road Condition: Dry, Consatruction Zone: True] \n""")
    ...
    # Calculate probs of Construction Zone against C1 & C2
    print(" --- Probability Construction Zone against C1 & C2 --- \n")

    num_construction_nca = crash_data_frame[crash_data_frame["Driver Action"] == "No Contributing Action"]["Construction Zone"].value_counts().get(True)
    prob_construction_nca = (num_construction_nca / num_nca)

    num_construction_ca = crash_data_frame[crash_data_frame["Driver Action"] != "No Contributing Action"]["Construction Zone"].value_counts().get(True)
    prob_construction_ca = (num_construction_ca / num_ca)

    print(f" P(Construction Zone | No Contributing Factor): {prob_construction_nca}")
    print(f" P(Construction Zone | Contributing Factor): {prob_construction_ca} \n")
    ...
    prob_x_nca = (prob_construction_nca * prob_clear_nca * prob_dry_nca)
    prob_x_ca = (prob_construction_ca * prob_clear_ca * prob_dry_ca)
    ...
    if (prob_x_nca_prob_nca > prob_x_ca_prob_ca):
        print(" RESULT: Given the crash scenario X = [Weather Condition: Clear, Road Condition: Dry, Construction Zone: True] we predict there will be No Contributing Action from the driver.")
    else:
        print(" RESULT: Given the crash scenario X = [Weather Condition: Clear, Road Condition: Dry, Construction Zone: True] we predict there will be a Contributing Action from the driver.")
